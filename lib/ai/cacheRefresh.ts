...
function parseRefreshResponse(response: OpenAIResponse): RefreshResponse {
  ...
  try {
    ...
    return { opportunities: [], parseFailed: true };
  }
}
...
function promptLine(value?: string | null) {
  return String(value ?? "").replace(/[\r\n]+/g, " ").replace(/\s+/g, " ").trim().slice(0, 180);
}

function formatExistingCachedOpportunities(existingOpportunities: CachedBucketOpportunity[]) {
  if (!existingOpportunities.length) return "- None found for this bucket.";
  return existingOpportunities.slice(0, 20).map((item) => {
    const status = promptLine(item.reviewStatus || "pending");
    const company = promptLine(item.company || "Unknown company");
    const title = promptLine(item.title || "Unknown title");
    const url = promptLine(item.url || "No URL");
    const deadline = item.deadline ? ` | deadline: ${promptLine(item.deadline)}` : "";
    return `- ${status} | ${company} | ${title} | ${url}${deadline}`;
  }).join("\n");
}
...
export async function refreshBucketOpportunities(bucket: SearchBucket, refreshRunId: string, limit: number, existingOpportunities: CachedBucketOpportunity[] = []) {
  const model = process.env.OPENAI_MODEL || "gpt-4.1-mini";
  const existingCacheList = formatExistingCachedOpportunities(existingOpportunities);
  const prompt = `Find 2-3 high-quality current internship, trainee, graduate internship or student placement opportunities for business school students.\n\nThe goal is not to maximize quantity. The goal is to find opportunities that would make a student think: "this is genuinely relevant and attractive."\n\nBucket:\n- id: ${bucket.id}\n- category: ${bucket.category.name}\n- region/market: ${bucket.region}\n- track title: ${bucket.displayTitle}\n\nExisting cached opportunities to avoid:\n${existingCacheList}\n\nDuplicate avoidance rules:\n- Do not return opportunities that are already present in this bucket cache.\n- Treat an opportunity as duplicate if it has the same URL, the same or very similar title and same company, or the same company plus the same role family.\n- If an existing opportunity is approved, do not propose it again.\n- If an existing opportunity is rejected, do not propose it again.\n- Find a genuinely new opportunity for this bucket.\n\nSearch strategy:\n- Search like a strong human internship researcher.\n- Prefer direct employer career pages and official ATS pages such as Greenhouse, Lever, Workday, Teamtailor, SmartRecruiters, Ashby or company job pages.\n- Prioritize prestigious, recognizable, high-signal companies or organizations.\n- For business school students, prefer global brands, well-known corporates, top consulting firms, investment banks, recognized startups or scaleups, major sports organizations, luxury groups, major consumer goods companies, reputable tech companies, and companies that look strong on a CV.\n- Avoid low-signal unknown companies unless the role is exceptionally relevant and credible.\n- Prefer reputable companies, recognized organizations, strong brands, high-growth startups, sports organizations, international institutions, consulting firms, finance firms, tech companies, hospitality groups or other employers that business school students would consider attractive.\n- Prefer 4-6 month or 6-month internships when possible.\n- Prefer roles relevant to business school profiles: strategy, marketing, partnerships, sponsorship, sales, finance, operations, events, e-commerce, data/business analytics, project management or international business depending on the bucket.\n\nStrict rejection rules:\n- Reject generic career pages, generic internship program pages, talent community pages, job search result pages and company careers homepages.\n- Reject vague pages titled like "Internships at Company", "Students and graduates", "Early careers", "Careers", "Jobs at Company", "Open positions" or "Graduate opportunities".\n- Reject aggregator pages that do not link to a direct employer or ATS job posting.\n- Reject LinkedIn apply pages, LinkedIn search pages, and LinkedIn job pages.\n- Reject old or expired postings.\n- Reject pages where the URL is only a generic database or search page.\n- Reject low-quality scraper sites unless they provide a direct official job URL.\n- Reject any opportunity that does not have a specific role title, a specific company, a specific location or clear remote/hybrid location, a direct URL to the specific job posting, and evidence that applications are currently open.\n- Every returned url must be the direct employer or ATS job posting URL, not an aggregator URL.\n- Acceptable URLs include company Workday job pages, Greenhouse job pages, Lever job pages, Teamtailor job pages, SmartRecruiters job pages, Ashby job pages, and company careers job detail pages.\n- Unacceptable URLs include datainternships.co, ceruleanjobs.com, LinkedIn, search result pages, and generic job board pages without a direct employer posting.\n- Reject clearly unpaid internships.\n- Accept paid, stipend, allowance, or compensation not specified if the employer/opportunity is strong.\n- If compensation is not specified, include exactly this risk: "Compensation not specified; confirm before applying."\n- Reject any offer without a direct usable URL.\n- Reject generic search result URLs, LinkedIn search URLs, Google URLs or pages that are not an actual specific job posting.\n- Reject senior roles, manager roles, full-time permanent roles and non-internship roles.\n- Reject roles where the deadline is clearly in the past.\n- Do not reject or downgrade an opportunity only because the deadline is close if applications are still open. Add exactly this risk instead: "Deadline is close; apply quickly."\n- If the deadline is unclear, accept only if the URL is a specific job posting and the opportunity is strong. Add exactly this risk: "Deadline unclear; verify before applying."\n- Reject low-quality filler opportunities. It is better to return 1-2 strong opportunities than 3 mediocre ones.\n\nScore calibration:\n- Do not give very high scores to vague or generic opportunities. Reject them instead.\n- qualityScore should consider employer prestige, brand recognition, role credibility, direct application URL, and freshness.\n- A prestigious company with a clear internship can score higher.\n- An unknown company should not receive 90+ quality unless the opportunity is extremely strong.\n- Quality score above 90 requires a direct specific job posting URL, clearly open application, strong employer, strong bucket fit, and clear internship/trainee/student placement status.\n- Do not cap quality score only because a still-open deadline is close.\n\nOutput rules:\n- Return only valid JSON matching the schema.\n- Strings must not contain raw newline characters. Use escaped backslash-n sequences or replace line breaks with spaces.\n- Escape quotes and line breaks properly inside every string value.\n- descriptionSummary must be 200 characters or fewer.\n- requirementsSummary must be 200 characters or fewer.\n- applicationAngle must be 200 characters or fewer.\n- linkedinMessage must be 200 characters or fewer.\n- coverLetterHook must be 200 characters or fewer.\n- rawSourceSnippet must be a short plain-text evidence string, max 120 characters.\n- Do not include markdown in rawSourceSnippet.\n- Do not include links in rawSourceSnippet.\n- Do not include line breaks in rawSourceSnippet.\n- Do not copy website text. Summarize the evidence in plain text.\n- Example good rawSourceSnippet: "Direct job posting shows Apply button and 2026 internship title."\n- Do not copy long raw snippets from websites.\n- Every opportunity must include a direct URL to a specific employer or ATS job posting.\n- Every opportunity must include why it matches the bucket.\n- Every opportunity must include risks, especially if compensation or deadline is unclear.\n- Scores should be realistic. Do not give 95+ scores unless the opportunity is exceptionally strong.`;
  ...
  const response = await createRefreshResponse({
    ...
    input: [
      { role: "system", content: "You are a careful internship cache refresh researcher. You validate URLs, avoid duplicates already present in the bucket cache, prioritize prestigious employers, and reject generic pages, weak, expired, unpaid, senior or non-internship results. Quality matters more than quantity. Return only valid JSON with no raw newline characters inside strings." },
  ...
  if (parsed.opportunities.length === 0 && !parsed.parseFailed) {
    Sentry.withScope((scope) => {
      scope.setTag("feature", "cache-refresh");
  ...
  } else if (!parsed.parseFailed && validatedOpportunities.length === 0) {
    Sentry.withScope((scope) => {
      scope.setTag("feature", "cache-refresh");
      scope.setTag("bucketId", bucket.id);
