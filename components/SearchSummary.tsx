...
export function SearchSummary({ profile }: { profile: CandidateProfile }) {
  const rows = [
    ["Target market", profile.targetCountries.join(", ")],
    ["Cities", profile.targetCities.join(", ") || "Flexible"],
    ["Internship tracks", profile.desiredRoles.join(", ")],
    ["Start", profile.internshipStartDate || "Flexible"],
    ["Duration", profile.internshipDuration || "Flexible"],
    ["Languages", profile.languagesSpoken.join(", ") || "Not specified"]
  ];
