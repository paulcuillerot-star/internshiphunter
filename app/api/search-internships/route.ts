...
export async function POST(request: Request) {
  ...
  const profile: CandidateProfile = {
    id: makeId("profile"), firstName: String(formData.get("firstName") ?? ""), email: String(formData.get("email") ?? ""), cvFileUrl: cvFileName,
  ...
  try {
    ...
    const report: InternshipSearchReport = { id: reportId, profileId: profile.id, status: "completed", isPaid: false, freeOffers: result.offers.slice(0, 2).map((offer) => ({ ...offer, isPremium: false })), premiumOffers: result.offers.slice(2, 7).map((offer) => ({ ...offer, isPremium: true })), createdAt: now, updatedAt: new Date().toISOString() };
    saveReport(report);
    saveLog({ id: makeId("log"), profileId: profile.id, reportId, status: "completed", querySummary: result.querySummary, rawResponse: result.rawResponse, createdAt: new Date().toISOString() });
    ...
    saveReport(report);
    saveLog({ id: makeId("log"), profileId: profile.id, reportId, status: "failed", querySummary: "Search failed before completion.", errorMessage: message, createdAt: new Date().toISOString() });
