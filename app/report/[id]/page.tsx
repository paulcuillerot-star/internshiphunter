    ...
    <section className="section">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h1 className="mt-3 text-4xl font-bold text-ink">Your first 2 golden offers</h1>
      ...
      <div className="mt-8">
        <SearchSummary profile={profile} />
      </div>

      <div className="mt-8 grid gap-5">
        {report.freeOffers.map((offer) => (
          <OfferCard key={offer.id} offer={offer} reportId={report.id} />
        ))}
      </div>

      <div className="mt-10 grid gap-5 lg:grid-cols-[1fr_0.75fr]">
        <div className="grid gap-5">
          {report.premiumOffers.map((offer, index) => (
            <LockedOfferCard key={offer.id} index={index + 1} />
          ))}
        </div>
